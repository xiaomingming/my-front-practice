...
(function(window, undefined) {
    ...
    ckUtils.getCookie = function(key) {
    ...
    // cookie expires时间获取
    ckUtils.getExpires = function(time) {
        /*if (!this.isValidTime(time)) {
            throw new Error('error fomart of time');
        }*/
        return time
    };
    // 获取设置
    ...
    ckUtils.getCookieConfig = function(key, value, opts) {
        ...
        str = key + '=' + value;
        str += opts.expires ? ';expires=' + this.getExpires(opts.expires) : '';
    ...
    // 判断设置的时间正确性
    /*ckUtils.isValidTime = function(time) {
        return Object.prototype.toString.call(time).slice(8, -1) === 'Date' && !isNaN(new Date(time).getTime());
    };*/
    // 删除cookie
    ckUtils.delCookie = function(key) {
